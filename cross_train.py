...
from models import rnn_with_feature
...
for i in range(2,10):
    ...
    model = rnn_with_feature(sequence_shape=(None, 3),feature_shape=(2,), num_classes=11, dropout_rate=0.4)
    # model.summary()
    ...
    keras.callbacks.ModelCheckpoint(
        ...
        filepath='models/aug_to_max_fold%d/model_{epoch}'%i,
        save_freq='epoch'),
    # keras.callbacks.TensorBoard(log_dir='models/weighted_focal_loss_fold%d/logs'%i)
    keras.callbacks.TensorBoard(log_dir='models/aug_to_max_fold%d/logs'%i)
    ]
